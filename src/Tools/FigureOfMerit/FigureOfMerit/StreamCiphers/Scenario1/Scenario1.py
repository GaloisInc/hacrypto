...
class Scenario1:
    ...
    def get_cipher_implementation(self, name, state_size, key_size, iv_size, implementation_version,
        ...
        for cipher_implementation in self.ciphers_implementations:
            if cipher_implementation.name == name and cipher_implementation.state_size == state_size and \
                            cipher_implementation.key_size == key_size and \
                            cipher_implementation.iv_size == iv_size and \
    ...
    def get_selected_cipher(self, name, state_size, key_size, iv_size):
        """
        Get selected cipher by given characteristics
        :param name: Cipher name
        :param name: Cipher state size
        :param name: Cipher key size
        :param name: Cipher IV size
        :return: Searched cipher if found, else false
        """

        for selected_cipher in self.selected_ciphers:
            if selected_cipher.name == name and selected_cipher.state_size == state_size and \
                            selected_cipher.key_size == key_size and selected_cipher.iv_size == iv_size:
    ...
    def read_csv_file(self, architecture):
        ...
        with open(file) as csv_file:
            ...
            for row in reader:
                ...
                execution_time_s = int(row[Constants.EXECUTION_TIME_S_COLUMN_INDEX])
                ...
                cipher_implementation = self.get_cipher_implementation(name, state_size, key_size, iv_size,
                                                                       implementation_version,
                                                                       implementation_compiler_options)
                if not cipher_implementation:
                    cipher_implementation = CipherImplementation(name, state_size, key_size, iv_size,
                                                                 implementation_version,
                                                                 implementation_compiler_options)
                ...
                selected_cipher = self.get_selected_cipher(name, state_size, key_size, iv_size)
                if not selected_cipher:
                    link = self.ciphers_info.get_info(name, state_size, key_size, iv_size)
                    selected_cipher = SelectedCipher(name, state_size, key_size, iv_size, link)
    ...
    def fom(self):
        ...
        for cipher_implementation in self.ciphers_implementations:
            ...
            value = cipher_implementation.avr_metrics.ram_data_total + max([
                cipher_implementation.avr_metrics.ram_stack_s,
            ...
            value = cipher_implementation.msp_metrics.ram_data_total + max([
                cipher_implementation.msp_metrics.ram_stack_s,
            ...
            value = cipher_implementation.arm_metrics.ram_data_total + max([
                cipher_implementation.arm_metrics.ram_stack_s,
            ...
            # Execution time
            value = cipher_implementation.avr_metrics.execution_time_s + \
            ...
            value = cipher_implementation.msp_metrics.execution_time_s + \
            ...
            value = cipher_implementation.arm_metrics.execution_time_s + \
        ...
        # AVR
        self.ciphers_implementations.sort(key=lambda c: c.fom_avr)
        ...
        for selected_cipher in self.selected_ciphers:
            for cipher_implementation in self.ciphers_implementations:
                if selected_cipher.name == cipher_implementation.name and \
                                selected_cipher.state_size == cipher_implementation.state_size and \
                                selected_cipher.key_size == cipher_implementation.key_size and \
                                selected_cipher.iv_size == cipher_implementation.iv_size:
        ...
        # MSP
        self.ciphers_implementations.sort(key=lambda c: c.fom_msp)
        ...
        # ARM
        self.ciphers_implementations.sort(key=lambda c: c.fom_arm)
        ...
        if Constants.RECOMPUTE_FOM:
            ...
            for selected_cipher in self.selected_ciphers:
                ...
                value = selected_cipher.avr_metrics.ram_data_total + max([
                    selected_cipher.avr_metrics.ram_stack_s,
                ...
                value = selected_cipher.msp_metrics.ram_data_total + max([
                    selected_cipher.msp_metrics.ram_stack_s,
                ...
                value = selected_cipher.arm_metrics.ram_data_total + max([
                    selected_cipher.arm_metrics.ram_stack_s,
                ...
                # Execution time
                value = selected_cipher.avr_metrics.execution_time_s + \
                ...
                value = selected_cipher.msp_metrics.execution_time_s + \
                ...
                value = selected_cipher.arm_metrics.execution_time_s + \
    ...
    def generate_latex_table(self):
        ...
        # f.write('\\scriptsize \n')
        # f.write('\\begin{center} \n')
        f.write('\\caption {Results for scenario 1. Encrypt 128 bytes of data. For each cipher, an optimal '
                'implementation on each architecture is selected.} \label{tab:scen1} \n')
        ...
        # First section
        f.write('\\midrule \n')
        f.write('\\multicolumn{11}{c}{I: Setup + Encryption} \\\\ \n')
        f.write('\\midrule \n')
        ...
        for selected_cipher in self.selected_ciphers:
            ...
            avr_value2 = selected_cipher.avr_metrics.ram_data_total + max([
                selected_cipher.avr_metrics.ram_stack_s,
                selected_cipher.avr_metrics.ram_stack_e])
            avr_value3 = selected_cipher.avr_metrics.execution_time_s + \
                         selected_cipher.avr_metrics.execution_time_e
            ...
            msp_value2 = selected_cipher.msp_metrics.ram_data_total + max([
                selected_cipher.msp_metrics.ram_stack_s,
                selected_cipher.msp_metrics.ram_stack_e])
            msp_value3 = selected_cipher.msp_metrics.execution_time_s + \
                         selected_cipher.msp_metrics.execution_time_e
            ...
            arm_value2 = selected_cipher.arm_metrics.ram_data_total + max([
                selected_cipher.arm_metrics.ram_stack_s,
                selected_cipher.arm_metrics.ram_stack_e])
            arm_value3 = selected_cipher.arm_metrics.execution_time_s + \
                         selected_cipher.arm_metrics.execution_time_e
        ...
        # Second section
        f.write('\\midrule \n')
        f.write('\\multicolumn{11}{c}{II: Setup} \\\\ \n')
        f.write('\\midrule \n')
        ...
        for selected_cipher in self.selected_ciphers:
            # Code size
            value = selected_cipher.avr_metrics.code_size_s
            if avr_min_code_size > value:
                avr_min_code_size = value
            if avr_max_code_size < value:
                avr_max_code_size = value

            value = selected_cipher.msp_metrics.code_size_s
            if msp_min_code_size > value:
                msp_min_code_size = value
            if msp_max_code_size < value:
                msp_max_code_size = value

            value = selected_cipher.arm_metrics.code_size_s
            if arm_min_code_size > value:
                arm_min_code_size = value
            if arm_max_code_size < value:
                arm_max_code_size = value
            ...
            # RAM
            value = selected_cipher.avr_metrics.ram_data_s + \
                    selected_cipher.avr_metrics.ram_stack_s
            if avr_min_ram > value:
                avr_min_ram = value
            if avr_max_ram < value:
            ...
            value = selected_cipher.msp_metrics.ram_data_s + \
                    selected_cipher.msp_metrics.ram_stack_s
            if msp_min_ram > value:
                msp_min_ram = value
            if msp_max_ram < value:
            ...
            value = selected_cipher.arm_metrics.ram_data_s + \
                    selected_cipher.arm_metrics.ram_stack_s
            if arm_min_ram > value:
                arm_min_ram = value
            if arm_max_ram < value:
        ...
        for selected_cipher in self.selected_ciphers:
            avr_value1 = selected_cipher.avr_metrics.code_size_s
            avr_value2 = selected_cipher.avr_metrics.ram_data_s + \
                         selected_cipher.avr_metrics.ram_stack_s
            avr_value3 = selected_cipher.avr_metrics.execution_time_s

            msp_value1 = selected_cipher.msp_metrics.code_size_s
            msp_value2 = selected_cipher.msp_metrics.ram_data_s + \
                         selected_cipher.msp_metrics.ram_stack_s
            msp_value3 = selected_cipher.msp_metrics.execution_time_s

            arm_value1 = selected_cipher.arm_metrics.code_size_s
            arm_value2 = selected_cipher.arm_metrics.ram_data_s + \
                         selected_cipher.arm_metrics.ram_stack_s
            arm_value3 = selected_cipher.arm_metrics.execution_time_s
        ...
        # Third section
        f.write('\\midrule \n')
        f.write('\\multicolumn{11}{c}{III: Encryption} \\\\ \n')
        f.write('\\midrule \n')
    ...
    def generate_mediawiki_table(self):
        ...
        f.write('|+ Results for scenario 1 - I: Setup + Encryption. Encrypt 128 bytes of data. For each cipher, an '
                'optimal implementation on each architecture is selected. \n')
        f.write('|- \n')
        f.write('! scope="col" colspan="4" | Cipher Info \n')
        ...
        f.write('! scope="col" | State [b] \n')
        f.write('! scope="col" | Key [b] \n')
        f.write('! scope="col" | IV [b] \n')
        ...
        for selected_cipher in self.selected_ciphers:
            ...
            f.write(Constants.MEDIAWIKI_SECTION1_ROW_FORMAT.format(selected_cipher.name_link,
                                                                   selected_cipher.state_size,
                                                                   selected_cipher.key_size,
                                                                   selected_cipher.iv_size,
        ...
        # Table 2 header
        f.write('{| class="wikitable sortable" style="margin: auto;" \n')
        f.write('|+ Results for scenario 1 - II: Setup. For each cipher, an optimal implementation on each architecture'
                ' is selected. \n')
        f.write('|- \n')
        f.write('! scope="col" colspan="4" | Cipher Info \n')
        f.write('! scope="col" colspan="3" | AVR \n')
        ...
        # Table 3 header
        f.write('{| class="wikitable sortable" style="margin: auto;" \n')
        f.write('|+ Results for scenario 1 - III: Encryption. Encrypt 128 bytes of data. For each cipher, an optimal '
                'implementation on each architecture is selected. \n')
        f.write('|- \n')
        f.write('! scope="col" colspan="4" | Cipher Implementation \n')
        f.write('! scope="col" colspan="3" | AVR \n')
        f.write('! scope="col" colspan="3" | MSP \n')
        f.write('! scope="col" colspan="3" | ARM \n')
        f.write('|- \n')
