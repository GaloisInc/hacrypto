...
do
	...
	# Assume that the cipher is compliant from source file
	compliant=$TRUE
		...
		do
			...
			do
				...
				if [ $FALSE -eq $compliant_cipher_implementation_architecture_scenario ] ; then
					...
					if [ $SCRIPT_VERBOSITY_0 -ne $SCRIPT_VERBOSITY ] ; then
						echo "$(tput setab 2)$directory - architecture: $architecture; scenariro: $scenario - $(tput sgr 0) $(tput setaf 2)OK!$(tput sgr 0)"
					fi
				fi
			done
		done
