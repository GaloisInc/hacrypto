...
void Update(uint8_t *state, uint8_t *t1, uint8_t *t2, uint8_t *t3, uint8_t *stream)
{
	...
	*t1 = x1 ^ x2;
	...
	x2 = (state[21] << 4) ^ (state[22] >> 4);

	*t2 = x1 ^ x2;


	x1 = (state[30] << 2) ^ (state[31] >> 6);
	x2 = (state[35] << 7) ^ (state[36] >> 1);
	...
	*stream ^= *t1 ^ *t2 ^ *t3;
	...
	x3 = (state[20] << 6) ^ (state[21] >> 2);

	*t1 ^= (x1 & x2) ^ x3;


	x1 = (state[21] << 2) ^ (state[22] >> 6);
	x2 = (state[21] << 3) ^ (state[22] >> 5);
	x3 = (state[32] << 7) ^ (state[33] >> 1);

	*t2 ^= (x1 & x2) ^ x3;


	x1 = (state[35] << 5) ^ (state[36] >> 3);
	x2 = (state[35] << 6) ^ (state[36] >> 2);
	x3 = (state[7] << 5) ^ (state[8] >> 3);
