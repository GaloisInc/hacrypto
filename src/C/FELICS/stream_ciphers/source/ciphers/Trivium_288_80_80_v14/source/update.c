...
void Update(uint8_t *state, uint16_t *t1, uint16_t *t2, uint16_t *t3, uint16_t *stream)
{
	...
	*t1 = x1 ^ x4;
	...
	x4 = (State[10] << 4) ^ (State[11] >> 12);

	*t2 = x1 ^ x4;


	x1 = (State[14] << 10) ^ (State[15] >> 6);
	x4 = (State[17] << 7) ^ ((uint16_t)state[36] >> 1);
	...
	*stream ^= *t1 ^ *t2 ^ *t3;
	...
	x5 = (State[9] << 14) ^ (State[10] >> 2);

	*t1 ^= (x2 & x3) ^ x5;


	x2 = (State[10] << 2) ^ (State[11] >> 14);
	x3 = (State[10] << 3) ^ (State[11] >> 13);
	x5 = (State[15] << 15) ^ (State[16] >> 1);

	*t2 ^= (x2 & x3) ^ x5;


	x2 = (State[17] << 5) ^ ((uint16_t)state[36] >> 3);
	x3 = (State[17] << 6) ^ ((uint16_t)state[36] >> 2);
