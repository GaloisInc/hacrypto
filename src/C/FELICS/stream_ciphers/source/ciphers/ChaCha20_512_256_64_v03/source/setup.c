...
void Setup(uint8_t *state, uint8_t *key, uint8_t *iv)
{
	...
	/* First row: SIGMA */
	State[0] = READ_CONSTANT_DOUBLE_WORD(SIGMA[0]);
	State[1] = READ_CONSTANT_DOUBLE_WORD(SIGMA[1]);
	State[2] = READ_CONSTANT_DOUBLE_WORD(SIGMA[2]);
	State[3] = READ_CONSTANT_DOUBLE_WORD(SIGMA[3]);
