...
void Encrypt(uint8_t *block, uint8_t *roundKeys)
{
	asm volatile (\
		...
		"	LEA_EncBlk_1Rnd R7, R4, R5, R6, R9, R10, R11, R8, R12, R13, R14, R15, R0, R1, R2, R3;\n"
		"	LEA_EncBlk_1Rnd R8, R9, R10, R11, R13, R14, R15, R12, R0, R1, R2, R3, R7, R4, R5, R6;\n"
		"	LEA_EncBlk_1Rnd R12, R13, R14, R15, R1, R2, R3, R0, R7, R4, R5, R6, R8, R9, R10, R11;\n"
...
void Encrypt(uint8_t *block, uint8_t *roundKeys)
{
	...
	for (i = 0; i < NUMBER_OF_ROUNDS; i += 4, rk += 4) {
		...
		rk +=4;
		b0 = rotr((b3 ^ RK(rk, RV4)) + (b0 ^ RK(rk, RVC)), 3);
		b3 = rotr((b2 ^ RK(rk, RV2)) + (b3 ^ RK(rk, RVC)), 5);
		b2 = rotl((b1 ^ RK(rk, RV0)) + (b2 ^ RK(rk, RVC)), 9);
		
		rk +=4;
		b1 = rotr((b0 ^ RK(rk, RV4)) + (b1 ^ RK(rk, RVC)), 3);
		b0 = rotr((b3 ^ RK(rk, RV2)) + (b0 ^ RK(rk, RVC)), 5);
		b3 = rotl((b2 ^ RK(rk, RV0)) + (b3 ^ RK(rk, RVC)), 9);
		
		rk +=4;
		b2 = rotr((b1 ^ RK(rk, RV4)) + (b2 ^ RK(rk, RVC)), 3);
		b1 = rotr((b0 ^ RK(rk, RV2)) + (b1 ^ RK(rk, RVC)), 5);
		b0 = rotl((b3 ^ RK(rk, RV0)) + (b0 ^ RK(rk, RVC)), 9);
	}
