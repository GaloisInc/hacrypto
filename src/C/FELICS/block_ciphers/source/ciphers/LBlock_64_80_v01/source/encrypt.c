...
void EncryptRound(uint8_t x[8], uint8_t k[4])
{
	...
	x[7] = ((READ_SBOX_BYTE(S7[((x[7] >> 4) & 0x0F)])) << 4) ^ READ_SBOX_BYTE(S6[(x[7] & 0x0F)]);
	x[6] = ((READ_SBOX_BYTE(S5[((x[6] >> 4) & 0x0F)])) << 4) ^ READ_SBOX_BYTE(S4[(x[6] & 0x0F)]);
	x[5] = ((READ_SBOX_BYTE(S3[((x[5] >> 4) & 0x0F)])) << 4) ^ READ_SBOX_BYTE(S2[(x[5] & 0x0F)]);
	x[4] = ((READ_SBOX_BYTE(S1[((x[4] >> 4) & 0x0F)])) << 4) ^ READ_SBOX_BYTE(S0[(x[4] & 0x0F)]);
