...
void encryption_round_function(uint8_t *data,uint8_t *rkey,uint8_t round,uint8_t *temp){
	uint8_t i;
...
void Encrypt(uint8_t *block, uint8_t *roundKeys)
{
	...
	for(i=0;i<(NUMBER_OF_ROUNDS-1);i++) encryption_round_function(block,roundKeys+BLOCK_SIZE,i,&temp);
	for(i=0;i<4;i++){
