...
void DecryptRound(uint8_t x[8], uint8_t k[4])
{
	...
	/* XOR X right half with the round key: X XOR K(j) */
	...
	x[3] = ((READ_SBOX_BYTE(S7[((x[3] >> 4) & 0x0F)])) << 4) ^ READ_SBOX_BYTE(S6[(x[3] & 0x0F)]);    
	x[2] = ((READ_SBOX_BYTE(S5[((x[2] >> 4) & 0x0F)])) << 4) ^ READ_SBOX_BYTE(S4[(x[2] & 0x0F)]);
	x[1] = ((READ_SBOX_BYTE(S3[((x[1] >> 4) & 0x0F)])) << 4) ^ READ_SBOX_BYTE(S2[(x[1] & 0x0F)]);
	x[0] = ((READ_SBOX_BYTE(S1[((x[0] >> 4) & 0x0F)])) << 4) ^ READ_SBOX_BYTE(S0[(x[0] & 0x0F)]);

	
	/* (3) Diffusion function P: P(S(X XOR K(j))) */
	p[3] = ((x[3] & 0x0F) << 4) ^ (x[2] & 0x0F);
	p[2] = (x[3] & 0xF0) ^ ((x[2] >> 4) & 0x0F);
	p[1] = ((x[1] & 0x0F) << 4) ^ (x[0] & 0x0F);
	p[0] = (x[1] & 0xF0) ^ ((x[0] >> 4) & 0x0F);
