...
void Decrypt(uint8_t *block, uint8_t *roundKeys)
{
	...
	for ( i = 0; i < NUMBER_OF_ROUNDS; i++ ) {
		...
		s1 ^= READ_ROUND_KEY_BYTE(*(roundKeys+1));
		s2 ^= READ_ROUND_KEY_BYTE(*(roundKeys+2));
		s3 ^= READ_ROUND_KEY_BYTE(*(roundKeys+3));
		s4 ^= READ_ROUND_KEY_BYTE(*(roundKeys+4));
		s5 ^= READ_ROUND_KEY_BYTE(*(roundKeys+5));
		s6 ^= READ_ROUND_KEY_BYTE(*(roundKeys+6));
		s7 ^= READ_ROUND_KEY_BYTE(*(roundKeys+7));
		roundKeys -= 8;
