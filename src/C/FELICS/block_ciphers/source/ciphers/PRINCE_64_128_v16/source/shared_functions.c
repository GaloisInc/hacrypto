...
void AddRoundRoundKeyAndRoundConstant(uint8_t *block, uint8_t *roundKey, const uint8_t *roundConstant)
{
	...
	uint32_t *RoundKey = (uint32_t *)roundKey;
	uint32_t *RoundConstant = (uint32_t *)roundConstant;


	for(i = 0; i < 2; i++)
	{
		Block[i] = Block[i] ^ READ_ROUND_CONSTANT_DOUBLE_WORD(RoundConstant[i]);
		Block[i] = Block[i] ^ READ_ROUND_KEY_DOUBLE_WORD(RoundKey[i]);
	}
}
