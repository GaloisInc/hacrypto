...
void benes(vec128 * r, const unsigned char * bits, int rev)
{
	int i, iter, inc;
	...
	for (i = 0; i < 64; i++)
	{
		r_int_v[0][i] = vec128_extract(r[i], 0);
		r_int_v[1][i] = vec128_extract(r[i], 1);
	}
	...
	for (i = 0; i < 64; i++)
		r[i] = vec128_set2x(r_int_v[0][i], r_int_v[1][i]);
}
