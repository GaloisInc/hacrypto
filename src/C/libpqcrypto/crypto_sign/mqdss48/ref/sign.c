                ...
                const unsigned char *sk)
{
    ...
    memmove(sm + SIG_LEN - SEED_BYTES, sk, SEED_BYTES);
    ...
    memmove(sm + SIG_LEN - HASH_BYTES - PK_BYTES, pk, PK_BYTES);
    memmove(sm + SIG_LEN - HASH_BYTES, sm, HASH_BYTES);
                     ...
                     const unsigned char *pk)
{
    ...
    memmove(sigcopy, sm, SIG_LEN);
    memmove(m + PK_BYTES + HASH_BYTES, sm + SIG_LEN, smlen - SIG_LEN);  // Copy message.
    sm = sigcopy;

    memmove(m, pk, PK_BYTES);  // Copy pk to m.
    memmove(m + PK_BYTES, sm, HASH_BYTES);  // Copy R to m.
    ...
    memmove(sigma0, sm, HASH_BYTES);

    shake256_absorb(shakestate, D_sigma0_h0_sigma1, 2 * HASH_BYTES);
    shake256_squeezeblocks(shakeblock, 1, shakestate);

    memmove(h0, shakeblock, HASH_BYTES);

    sm += HASH_BYTES;

    memmove(t1packed, sm, ROUNDS * NPACKED_BYTES);
    sm += ROUNDS*NPACKED_BYTES;
    memmove(e1packed, sm, ROUNDS * MPACKED_BYTES);
    ...
    *mlen = smlen - SIG_LEN;
    memmove(m, m + PK_BYTES + HASH_BYTES, smlen - SIG_LEN);
    return 0;
}
